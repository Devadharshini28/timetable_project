	...
	<div class="container">
      <div class="">
		...
		<table id="employee_grid" class="table table-condensed table-hover table-striped" width="60%" cellspacing="0" data-toggle="bootgrid">
			<thead>
				<tr>
					<th data-column-id="Course_id" data-type="numeric" data-identifier="true">Course_id</th>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
				...
				<input type="hidden" value="0" name="edit_Course_id" id="edit_Course_id">
                  <div class="form-group">
                    <label for="name" class="control-label">Course_code:</label>
...
$( document ).ready(function() {
	var grid = $("#employee_grid").bootgrid({
		...
		formatters: {
		        ...
		        {
		            return "<button type=\"button\" class=\"btn btn-xs btn-default command-edit\" data-row-id=\"" + row.Course_id + "\"><span class=\"glyphicon glyphicon-edit\"></span></button> " + 
		                "<button type=\"button\" class=\"btn btn-xs btn-default command-delete\" data-row-id=\"" + row.Course_id + "\"><span class=\"glyphicon glyphicon-trash\"></span></button>";
		        }
		    }
   }).on("loaded.rs.jquery.bootgrid", function()
{
    ...
    grid.find(".command-edit").on("click", function(e)
    {
        //alert("You pressed edit on row: " + $(this).data("row-id"));
			var ele =$(this).parent();
			var g_Course_id = $(this).parent().siblings(':first').html();
            var g_Course_code = $(this).parent().siblings(':nth-of-type(2)').html();
console.log(g_Course_id);
		...
		$('#edit_model').modal('show');
					if($(this).data("row-id") >0) {
                                ...
                                // collect the data
                                $('#edit_Course_id').val(ele.siblings(':first').html()); // in case we're changing the key
                                $('#edit_Course_code').val(ele.siblings(':nth-of-type(2)').html());
                                $('#edit_Course_name').val(ele.siblings(':nth-of-type(3)').html());
